...
//IC检测命令
const UINT8 au8Icjc[12] = {0x24, 0x49, 0x43, 0x4a, 0x43, 0x00, 0x0c, 0x00, 0x00, 0x00, 0x00, 0x2b};
...
void BdSendCmdTXSQ(UINT8 *pSendAddr, UINT8 *pMsg, UINT8 u8MsgLen)
{
  ...
  pInfo = OSMemGet(pMediumMem, &err);
  if(err == OS_NO_ERR)
  {
    Uart3SendString("#Alloc Samll Memory! Make Sure The Message Length Is Less Than 128!\r\n");
...
void BdSendCmdICJC(void)
{
  ...
  if(!BdIcDet())
  {
    Uart3SendString("#BeiDou IC Card Not Found!\r\n ");
  }
 
  pRxBuf = OSMemGet(pSmallMem, &err);
  
  Uart1EmptyRxBuf();
  Uart1SendBuffer(au8Icjc, 12);
...
void CmdParseTXXX(void)
{
  ...
  Uart3SendString("#BeiDou Receiv One Massage: ");
  Uart3SendBuffer(&ptr[u8Field+2], u8Temp);
  Uart3SendString("! \r\n");
}
...
void CmdParseDWXX(void)
{
  ...
  Uart3SendBuffer(&sCmdDwxxCb.au8BdCmd[0], sizeof(sCmdDwxxCb));
}
...
void CmdParseFKXX(void )
{
   ...
   switch(sCmdFkxxCb.u8BackInfo)
   {
    case 0:
     {
       Uart3SendString("#BeiDou Send Massage Successful! \r\n");
       break;
     }
     
     case 1:
     {
       Uart3SendString("#BeiDou Send Massage Failed! \r\n");
       break;
     }
     
     case 2:
     {
       Uart3SendString("#BeiDou Signal Is Unlocked! \r\n");
       break;
     }
     
     case 3:
     {
       Uart3SendString("#BeiDou Battery Is Low! \r\n");
       break;
     }
     
     case 4:
     {
       Uart3SendString("#BeiDou Send Massage Time Don`t Come! \r\n");
       break;
     }
     
     case 5:
     {
       Uart3SendString("#BeiDou Encryption And Decryption Is Error! \r\n");
       break;
     }
     
     case 6:
     {
       Uart3SendString("#BeiDou Crc Check Is Error! \r\n");
       break;
     }
...
void BdCmdRun(void)
{
  ...
  while((Uart1GetRxDataLength() > 0) && (sBdCmdCb.eAssembleState != E_UART_CMD_RECEIVING_VALID))
  {
    BdCmdAssemble(Uart1GetByte());

    if(SwdEnable(sBdCmdCb.sBdDog) == TRUE)
    {
