...
void BdSendCmdICJC(void)
{
  ...
  while(TRUE)
  {
    if(Uart2GetRxDataLength()>21)
    {
      Uart2GetRxData(pRxBuf, 22);
...
void GetBufBdInfo(void)
{
  ...
  u8Len = Uart2GetRxDataLength();
  
  if(u8Len > 30)
  {
    Uart2GetRxData(ptr, u8Len);
...
void BdCmdRun(void)
{
  ...
  while((Uart2GetRxDataLength() > 0) && (sBdCmdCb.eAssembleState != E_UART_CMD_RECEIVING_VALID))
  {
    BdCmdAssemble(Uart2GetByte());
