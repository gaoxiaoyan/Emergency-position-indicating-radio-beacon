...
#define CMD_ASSERT(a)    ((a == CMD_ZCANS) || \
                          (a == CMD_ZCCNT) || \
                          (a == CMD_CMTI ) || \
                          (a == CMD_ZCEND) || \
                          (a == CMD_CSQ  ))
...
/*****************************************************************************
 ...
 返 回 值  : TRUE, FALSE
*****************************************************************************/
BOOL DeletCdmaMsg(UINT8 u8Index)
{
    UINT8   au8Buf[24];
    UINT8   i= 0;
    UINT8 * pMsg;

    if(((u8Index > 30) && (u8Index != 0xff)) || (u8Index < 0))
    {
        return FALSE;
    }
...
void CdmaCmdAssemble(UINT8 u8InData)
{
    switch(sCdmaCmdCb.eAssembleState)
    {
        case E_CDMA_RECEIVING_HEADER:
        {
            ...
            {
                if((u8InData == 'S') && (sCdmaCmdCb.u8MsgEnvent & 0x0f == 0x0f))
                {
                    sCdmaCmdCb.pu8AssembleBuf[0] = u8InData;
