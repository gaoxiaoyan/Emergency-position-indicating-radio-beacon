...
//定义电压下限为6v
#define REF_VOL         (2.85 * 10.0)       //基准电压2.85V
#define VOLTAGE_MIN     6.1       //最小电压6.5V
#define VOLTAGE_MAX     8.1       //最高电压8.4V
...
void BatteryDetect(void)
{
  ...
  if(bComplete == TRUE)
  {
    ...
    sSendInfo.u8Battery = (UINT8)((UINT32)((FLOAT32)u16AdcResules * REF_VOL + 0.5) >> 12); //ADC量化
...
UINT8 AdcDetect(BOOL bType)
{
  ...
  //电池电压为8.4v，经过3/13衰减，然后软件上全量程放大。然后进行四舍五入，得到剩余电量的百分比
  u8Bat = (UINT8)((UINT32)((FLOAT32)u16AdcResules * REF_VOL + 0.5) >> 12); //ADC量化
