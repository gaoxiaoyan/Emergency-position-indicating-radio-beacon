...
//返回：无
***************************************************************************/
void Uart0InsertByte(UINT8 u8Data)
{
    UART0_DISABLE_INTERRUPT();

    if(sUart0Cb.u16RxHeader > 0)
    {
        sUart0Cb.u16RxHeader--;
    }
    else
    {
        sUart0Cb.u16RxHeader = UART0_RX_MAX_SIZE - 1;
    }

    sUart0Cb.pu8RxBuf[sUart0Cb.u16RxHeader] = u8Data;
    sUart0Cb.u16RxDataLen++;
