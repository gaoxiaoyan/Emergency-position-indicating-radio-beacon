 ...
 * 描       述：
 * ----------------------------------------------------------------------------------------------------
 * 修改日期： 2016年1月6日10:31:06
 * 作    者： Bob
 * 当前版本： V1.01
 * 描    述:  1、为配合新版外壳，修改PCB, 更改
 * ----------------------------------------------------------------------------------------------------
...
void ReservedPinCfg(void)
{
  ...
  P9DIR &= ~(BIT0 + BIT1 + BIT2 + BIT3 + BIT7);
  P9OUT &= ~(BIT0 + BIT1 + BIT2 + BIT3 + BIT7);

  P10DIR &= ~(BIT0 + BIT1 + BIT2 + BIT3 + BIT6 + BIT7);
  P10OUT &= ~(BIT0 + BIT1 + BIT2 + BIT3 + BIT6 + BIT7);
 ...
 *返回:
*****************************************************************************/
void FlashLedEnable(void)
{
    StopWatchDog();             //停止看门狗

    PicFlashLedOn();           //低电平
    g_u8TimerDelay = 0;
    while(g_u8TimerDelay < 20);    //低电平时间超过100ms则启动LED闪烁
    PicFlashLedOff();            //高电平
 ...
 *返回:
*****************************************************************************/
void FlashLedDisable(void)
{
    StopWatchDog();             //停止看门狗

    PicFlashLedOn();           //低电平
    g_u8TimerDelay = 0;
    while(g_u8TimerDelay < 100);    //低电平时间超过200ms则关闭LED闪烁
    PicFlashLedOff();            //高电平
...
void SystemInit(void)
{
  ...
  //低压指示灯
  LowPowerRedEnable();
  LowPowerRedDir |= LowPowerRed;
  LowPowerBlueEnable();
  LowPowerBlueDir |= LowPowerBlue;
  LowPowerGreenEnable();
  LowPowerGreenDir |= LowPowerGreen;

  //CDMA指示灯
  CdmaIndicatorDisable();
  CdmaIndicatorDir |= CdmaIndicator;
...
/***************************************************************************
//函数名称：AutoSystemGapTime
//功能描述：根据系统已经运行的时间调整间隔时间
//
//参数：无
//返回：无
***************************************************************************/
#define DAY_MINUTE    (60 * 24)
void AutoSystemGapTime(void)
{
    static UINT8 u8OldDay=0;
    UINT8 u8Day=0;
    UINT8 u8Temp=0;

    u8Day = (UINT8)(sRecordCb.u32SysRunTime / DAY_MINUTE);
    ...
    if(u8OldDay != u8Day)
    {
        ...
        if((sRecordCb.u8GapTime >= '1') && (sRecordCb.u8GapTime <= '6'))
        {
            ...
            sSendInfo.u8Minute  = u8Temp * 5;

            sRecordCb.u8GapTime = u8Temp + '0';
...
#if DEBUG_EN > 0
     Uart2SendString("Enter Low Power Mode.\r\n");
#endif
...
#if DEBUG_EN > 0
     Uart2SendString("Low Power Mode Wake Up.\r\n");
#endif
