...
void CreatCmdIndex(void)
{
  ...
  memset(sGpsCmdCb.pu8CmdIndex, 0, (BUF_SMALL_SIZE-1));
...
UINT8 GpsInfoProcess(UINT8 u8Cmd)
{
  ...
  if(u8Cmd == GNGGA )
  {
    ...
    else
    {
      ...
      {
        Uart2SendString("#Gps Receiving Invalid Data!\r\n");
      }
...
//      sGpsInfoBuf[u8Num].GPS_Y[10] = 0;
 ...
 //     sGpsInfoBuf[u8Num].GPS_X[11] = 0;
      ...
      {
        Uart2SendString("#========================================================================#\r\n");
        Uart2SendString("#The Current Position is: 纬度 <");
        Uart2SendBuffer(sGpsInfoBuf[u8Num].GPS_Y, 11);
        Uart2SendString(" >...经度 < ");
        Uart2SendBuffer(sGpsInfoBuf[u8Num].GPS_X, 12);
        Uart2SendString(" >....#\r\n");
        Uart2SendString("#========================================================================#\r\n");
      }
 ...
 //       sGpsCmdCb.u8GpsRxNum = 0;
        if(g_bDebug & GPS_DEBUG)
        {
          Uart2SendString("#Gps Receiving Data Complete!\r\n");
        }
...
/**********************************************************************************************
 ...
 返回： 转换后的浮点数
**********************************************************************************************/
FLOAT32 My_atof(UINT8 *s, UINT8 u8Len)
{
    UINT8 i,j,k=0;
    FLOAT32 total,head = 0.0,tail = 0.0;
...
//  UINT8   au8Buf[16];
  ...
  {
    for(k=1; k<sGpsCmdCb.u8GpsRxNum; k++)
    {
      for(j=0; j<sGpsCmdCb.u8GpsRxNum-k; j++)
      {
        if(memcmp(&sGpsInfoBuf[j].GPS_Y[0], &sGpsInfoBuf[j+1].GPS_Y[0], 10) > 0)
        {
          memcpy(&sGpsTemp.GPS_Y[0], &sGpsInfoBuf[j+1].GPS_Y[0], 10);
          memcpy(&sGpsInfoBuf[j+1].GPS_Y[0], &sGpsInfoBuf[j].GPS_Y[0], 10);
          memcpy(&sGpsInfoBuf[j].GPS_Y[0], &sGpsTemp.GPS_Y[0], 10);
        }
      }
    }
    ...
    for(k=1; k<sGpsCmdCb.u8GpsRxNum; k++)
    {
      for(j=0; j<sGpsCmdCb.u8GpsRxNum-k; j++)
      {
        if(memcmp(&sGpsInfoBuf[j].GPS_X[0], &sGpsInfoBuf[j+1].GPS_X[0], 11) > 0)
        {
          memcpy(&sGpsTemp.GPS_X[0], &sGpsInfoBuf[j+1].GPS_X[0], 11);
          memcpy(&sGpsInfoBuf[j+1].GPS_X[0], &sGpsInfoBuf[j].GPS_X[0], 11);
          memcpy(&sGpsInfoBuf[j].GPS_X[0], &sGpsTemp.GPS_X[0], 11);
        }
      }
    }
  }
  ...
  sGpsCmdCb.u8GpsRxNum = 0;
  //将有效数据存放到最后位置上
  memcpy(&sSendInfo.au8GPS_Y[0], &sGpsInfoBuf[GPS_MAX_SIZE/2].GPS_Y[0], 10);
  memcpy(&sSendInfo.au8GPS_X[0], &sGpsInfoBuf[GPS_MAX_SIZE/2].GPS_X[0], 11);
}
...
void GpsCmdRun(void)
{
 ...
 //获取有效数据包
  while((sGpsCmdCb.eAssembleState != E_UART_CMD_RECEIVING_VALID) && (Uart0GetRxDataLength() > 0))
  {
    GpsCmdAssemble(Uart0GetByte());

    if(SwdIsEnable(sGpsCmdCb.sGpsDog) == TRUE)
    {
  ...
  if(sGpsCmdCb.eAssembleState == E_UART_CMD_RECEIVING_VALID)
  {
    ...
    if(g_bGpsFirstRx == TRUE)   //控制GPS超时处理用，当接收GPS信息超过50秒后还
    {
      if(u8Cmd != GNNON)
      {
        if(g_bDebug & GPS_DEBUG)    //打印调试信息
        {
          Uart2SendBuffer(sGpsCmdCb.pu8AssembleBuf, sGpsCmdCb.u16BufDataLen);
        }

        CreatCmdIndex();            //建立索引表

        GpsStatus = GpsInfoProcess(u8Cmd);   //Gps数据处理

        if((GpsStatus == GPS_RX_VALID) && (bRtcFlag == FALSE))  //校准RTC
        {
    ...
    memset(sGpsCmdCb.pu8AssembleBuf, 0, sGpsCmdCb.u16BufDataLen);
    sGpsCmdCb.eAssembleState = E_UART_CMD_RECEIVING_HEADER;
    sGpsCmdCb.u16BufDataLen = 0;
  }
  else
