...
void Uart3Init(void)
{
    ...
    sUart3Cb.pu8RxBuf = au8RxBuffer3;              //初始化缓冲区结构体
...
/************************************************************************************
  ...
  返回值  ：无
*********************************************************************************/
void Uart3SendString(const UINT8 * pString)
{
  while(*pString != 0)
  {
    UCA3TXBUF = *pString;
...
/********************************************************************************
  ...
  返回：无
*********************************************************************************/
void Uart3SendBuffer(const UINT8 *pu8Buf, UINT8 u8Length)
{
  while(u8Length > 0)
  {
    UCA3TXBUF = *pu8Buf;
...
UINT16 Uart3GetRxData(UINT8 *pu8Buffer, UINT16 u16DataLen)
{
  ...
  {
    MEM_CPY(pu8Buffer, sUart3Cb.pu8RxBuf+sUart3Cb.u16RxHeader, u16DataLen);
    UART3_DISABLE_INTERRUPT();
...
void Uart3EmptyRxBuf(void)
{
  UART3_DISABLE_INTERRUPT();
  sUart3Cb.u16RxDataLen = 0;
  sUart3Cb.u16RxHeader = sUart3Cb.u16RxTail;
  UART3_ENABLE_INTERRUPT();
}
