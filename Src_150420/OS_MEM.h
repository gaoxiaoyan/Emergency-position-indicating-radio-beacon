...
#define OS_ENTER_CRITICAL()   _DINT()     //关闭全局中断控制位,进入临界区
#define OS_EXIT_CRITICAL()    _EINT()     //打开全局中断控制位,退出临界区
...
#define BUF_SMALL_SIZE         128

OS_EXT __no_init UINT8 au8LargeMem[4][BUF_LARGE_SIZE];
OS_EXT __no_init UINT8 au8MediumMem[4][BUF_MEDIUM_SIZE];
OS_EXT __no_init UINT8 au8SmallMem[16][BUF_SMALL_SIZE];
OS_EXT UINT8 err;
