...
//  CdmaIcCheckSetIn();
  ...
  SetCdmaOnOff(1);
  g_u16TimerDelay=0;
  while(g_u16TimerDelay < 500);  //延时5s，时间太短会检测不到手机卡
...
void CdmaCmdInit(void)
{
  ...
  SwdSetLifeTime(sCdmaCmdCb.sCdmaDog, 500);     //软件超时为0.5秒

  sCdmaCmdCb.sCsqDog       = SwdGet();
  SwdSetName(sCdmaCmdCb.sCsqDog, "CSQTimer");
  SwdSetLifeTime(sCdmaCmdCb.sCsqDog, 20000);     //每5秒获取一次信号强度
...
void CdmaCmdHandler(UINT8 u8Cmd)
{
    switch(u8Cmd)
    {
      ...
      case CMD_CMGR:
      {
          ...
          //如果短信箱中短信数量大于28条，则全部删除
          //如果短信接收量比较大，接收同时删除，会引起反馈信息混乱
          //则不能进行接收一条删除一条的操作
          if(s8CurrentIndex > 28)
          {
