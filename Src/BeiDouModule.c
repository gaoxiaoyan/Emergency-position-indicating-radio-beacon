...
void BdSendCmdTXSQ(UINT8 *pSendAddr, UINT8 *pMsg, UINT8 u8MsgLen)
{
	...
	//通讯申请命令组包
	i = 0;
	memcpy(&pInfo[i], "$TXSQ", 5);			        //5字节，输入发送命令
	i += 5;

	pInfo[i++] = (UINT8)(((UINT16)u8MsgLen + 18) >> 8);	//2字节，数据长度							//电文长度加上18个固定长度为通讯数据长度
	pInfo[i++] = (UINT8)( (UINT16)u8MsgLen + 18);

	memcpy(&pInfo[i], au8LocationAddr, 3);			//3字节用户地址											//获取本机卡号
	i += 3;

	pInfo[i++]  = 0x46;                                     //1字节，信息类别								                                    //ascii 为0x46；汉字编码为0x44

	memcpy(&pInfo[i], pSendAddr, 3);	                //3字节，用户地址                																	//发送方地址
	i += 3;

	pInfo[i++] = (UINT8)((UINT16)u8MsgLen * 8 >> 8);	//2字节，电文长度							  //电文信息长度，bit数
	pInfo[i++] = (UINT8)((UINT16)u8MsgLen * 8);

	pInfo[i++] = 0;					        //1字节，是否应答																					//应答位

	memcpy(&pInfo[i], pMsg, u8MsgLen);                      //＜210字节，电文内容
	i += u8MsgLen;
...
//检测一下是否插入IC卡
  ...
  if(!BdIcDet())
  {
    Uart2SendString("#BeiDou IC Card Not Found!\r\n ");
  }
#endif
...
void BdModuleInit(void)
{
   //IC检测
  BdSendCmdICJC();
  ReadSystemBdIndex(&sSendInfo.au8BDIndex[0], &au8LocationAddr[0]);
  BeiDouPowerOff();
