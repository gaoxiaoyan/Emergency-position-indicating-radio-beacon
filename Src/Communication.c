...
{
  //Uart2SendString("SendInfoToCenter!");
	UINT8 *ptr;
  UINT8 i;
//  _tsTimeCb sCurTime;
 ...
 // UINT8 au8X[11]    = {0x35,0x34,0x33,0x32,0x31,0x35,0x34,0x33,0x32,0x31,0x30};
  ...
  {
    SetRtcTime();
    Uart2SendString("g_bSendInfo == TRUE！");
    if(TRUE == g_bGpsCompleted)
    {
      Uart2SendString("TRUE == g_bGpsCompleted！");
      g_bSendInfo     = FALSE;
...
//      AssembleCdmaMsg(&sRecordCb.au8CdmaBckNbr[0], ptr, i);
        Uart2SendString("信息发送！");
      }
      else
      {
        if(g_bBdSend == TRUE)
        {
          ...
          Uart2SendString("北斗模式信息发送！");
#ifdef BEIDOU_TEST
          u16BdSendNum++;
          memset(&au8BdSendInfo[0], 0, sizeof(au8BdSendInfo));        //置位
