...
void CreatCmdIndex(void)
{
  ...
  MemClr(sGpsCmdCb.pu8CmdIndex, BUF_SMALL_SIZE);
...
//  sGpsCmdCb.bGpsParse = FALSE;
  ...
  sGpsCmdCb.pu8CmdIndex = OSMemGet(pSmallMem, &err2);
  ...
  SwdSetLifeTime(sGpsCmdCb.sGpsDog, 200);
   ...
   if(g_bDebug == TRUE)
   {
    if(err1 == OS_NO_ERR)
    {
      Uart3SendString("#GPS Assemble Buffer Get Successful！\r\n");
    }
    else
    {
      Uart3SendString("#GPS Assemble Buffer Get Fail！\r\n");
    }
    
    if(err2 == OS_NO_ERR)
    {
      Uart3SendString("#GPS CmdIndex Buffer Get Successful！\r\n");
    }
    else
    {
      Uart3SendString("#GPS CmdIndex Buffer Get Fail！\r\n");
    }
    
    if(sGpsCmdCb.sGpsDog == -1)
    {
      Uart3SendString("#GPS Assemble Software Timer Get Fail！\r\n");
    }
    else
    {
      Uart3SendString("#GPS Assemble Software Timer Get Successful！\r\n");
    }
   }
}
...
void GpsCmdRun(void)
{
 ...
 //获取有效数据包
  ...
  if(sGpsCmdCb.eAssembleState == E_UART_CMD_RECEIVING_VALID)
  {
    ...
    MemClr(sGpsCmdCb.pu8AssembleBuf, BUF_SMALL_SIZE);
    sGpsCmdCb.eAssembleState = E_UART_CMD_RECEIVING_HEADER;
    sGpsCmdCb.u16BufDataLen = 0;
  }
  else
