...
{  
  ...
  if(g_bDebug == TRUE)
  {
    Uart3SendString("#Get Power On SoftTimer, The Time Is Set To Three Second! \r\n");
  }
  ...
  while(TRUE)
  {
    if(!KeyPowerCheck)
    {
      if(SwdGetLifeTime(s8Dog) == 0)
      {
       ...
       {
         SystemVersionInfo();
         Uart3SendString("#System Power On! Release Power On SoftTimer! \r\n");
 ...
 * 返回值  ：无
****************************************************************/
void MemClr(UINT8 *pu8Buf,  UINT8 u8Len)
{
