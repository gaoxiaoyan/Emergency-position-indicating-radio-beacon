...
void Uart2Init(void)
{
    ...
    if(g_bDebug == TRUE)
    {
      if(err == OS_NO_ERR)
      {
        Uart3SendString("#UART2 Receiving Buffer Get Successful！\r\n");
      }
      else
      {
        Uart3SendString("#UART2 Receiving Buffer Get Fail！\r\n");
      }
    }
}
