...
{
  Uart2SendString("###############################################################\r\n");
  Uart2SendString("# Copyright (C) 2015~2016, QingDao Sunic-ocean.\r\n");
  ...
  Uart2SendString("#  9、cmd<08> -- 设置本台设备类型；\r\n");
  g_u8TimerDelay=0;
  while(g_u8TimerDelay < 15);    //低电平延时150ms
  Uart2SendString("# 10、cmd<09> -- 获取本台设备类型；\r\n");
  ...
  Uart2SendString("# 15、cmd<0e> -- 使用自动模式通讯；\r\n");

  Uart2SendString("# 16、cmd<10> -- GPS 调试信息关闭；\r\n");
  Uart2SendString("# 17、cmd<11> -- GPS 调试信息打开；\r\n");

  Uart2SendString("# 18、cmd<20> -- 北斗定位获取命令；\r\n");
  Uart2SendString("# 19、cmd<21> -- 北斗本机卡号获取；\r\n");
  g_u8TimerDelay=0;
  while(g_u8TimerDelay < 15);    //低电平延时150ms
  Uart2SendString("# 20、cmd<22> -- 北斗接收机号设置；\r\n");
  Uart2SendString("# 21、cmd<23> -- 北斗接收机号获取；\r\n");
  Uart2SendString("# 22、cmd<24> -- 北斗接收备用卡号设置；\r\n");
  Uart2SendString("# 23、cmd<25> -- 北斗接收备用卡号获取；\r\n");
  Uart2SendString("# 24、cmd<26> -- 北斗报文测试命令；\r\n");
  Uart2SendString("# 25、cmd<27> -- 北斗时间获取命令；\r\n");
  Uart2SendString("# 26、cmd<28> -- 北斗发送成功率获取；\r\n");
  Uart2SendString("# 27、cmd<29> -- 北斗发送成功率清零；\r\n");

  Uart2SendString("# 28、cmd<30> -- CDMA调试信息关闭；\r\n");
  Uart2SendString("# 29、cmd<31> -- CDMA调试信息打开；\r\n");
  g_u8TimerDelay=0;
  while(g_u8TimerDelay < 15);    //低电平延时150ms
  Uart2SendString("# 30、cmd<32> -- 通过CDMA获取时间；\r\n");
  Uart2SendString("# 31、cmd<33> -- 设置CDMA本机号码；\r\n");
  Uart2SendString("# 32、cmd<34> -- 获取CDMA本机号码；\r\n");
  Uart2SendString("# 33、cmd<35> -- 设置CDMA接收号码；\r\n");
  Uart2SendString("# 34、cmd<36> -- 获取CDMA接收号码；\r\n");
  Uart2SendString("# 35、cmd<37> -- 设置CDMA接收备用号码；\r\n");
  Uart2SendString("# 36、cmd<38> -- 获取CDMA接收备用号码；\r\n");
  Uart2SendString("# 37、cmd<39> -- 获取CDMA信号强度；\r\n");
  Uart2SendString("# 38、cmd<3A> -- CDMA短信发送测试；\r\n");

  Uart2SendString("# 39、cmd<18> -- 关闭LED闪烁功能；\r\n");
  Uart2SendString("# 40、cmd<19> -- 打开LED闪烁功能；\r\n");
...
void SetSystemType(void)
{
  SINT8 s8Dog;
  UINT8 au8Nbr[2];
  UINT8 u8SysType=0xff;    

  Uart2EmptyRxBuf();
  Uart2SendString("###############################################################\r\n");
  Uart2SendString("# Please Enter The Type Of This System: \r\n");
  Uart2SendString("#  01 -- 收油机；\r\n");
  Uart2SendString("#  02 -- 围油栏；\r\n");

  s8Dog = SwdGet();
  if(s8Dog < 0)
  {
    Uart2SendString("SetSystemType SoftTimer Is Out Of Scope!\r\n");
    return;
  }
  SwdSetLifeTime(s8Dog, 20000);
  SwdEnable(s8Dog);
  ...
  while(TRUE)
  {
    if(Uart2GetRxDataLength()>1)
    {
      ...
      if((memcmp(&au8Nbr[0], "99", 2)>0) || (memcmp(&au8Nbr[0], "01", 2) < 0))
      {
        Uart2SendString("# The Enter Type Is Beyond! Please Resend The Cmd!\r\n");
	Uart2SendString("###############################################################\r\n\r\n");
        SwdDisable(s8Dog);
        SwdFree(s8Dog);
        FreeWatchDog();          //释放看门狗
    ...
    if(SwdGetLifeTime(s8Dog) == 0)//长时间没收到命令，提示重发
    {
      SwdDisable(s8Dog);
      SwdFree(s8Dog);
      FreeWatchDog();          //释放看门狗
  ...
  //提取 设备类型
  u8SysType  = (au8Nbr[0 ] - '0') * 10;
  u8SysType += (au8Nbr[1 ] - '0') ;

  if(u8SysType != sRecordCb.u8SysType)
  {
    sRecordCb.u8SysType = u8SysType;
    g_u8NeedSaved |= SAVE_SYS_TYPE;
  }

  Uart2SendString("# System Type Set Successfull! \r\n");
  Uart2SendString("###############################################################\r\n\r\n");
}
...
void GetSystemType(void)
{
  ...
  au8Index[0] = sRecordCb.u8SysType%100/10 + '0';
  au8Index[1] = sRecordCb.u8SysType%10 + '0';
  au8Index[2] = '\r';
  au8Index[3] = '\n';

  Uart2SendString("###############################################################\r\n");
  Uart2SendString("# The Type Of This System Is: ");
  Uart2SendBuffer(&au8Index[0], 4);
  Uart2SendString("###############################################################\r\n\r\n");
}
...
void UseBeiDouMode(void)
{
    ...
    sSendInfo.u8MsgSource = BD_COM_MODE;
    g_u8NeedSaved |= SAVE_SYS_NBR;
}
...
发送： 0xAA + 0x55 + 0x0e + 0xCC + 0x33
*****************************************************************************/
void UseAutoMode(void)
{
    //将自动模式标志位置位，存到eeprom
    //查询CDMA信号强度，选择合适的通讯方式。
    if(g_bCdmaSend == TRUE)
    {
...
void CmdSetCdmaLocationNbr(void)
{
  ...
  Uart2SendString("###############################################################\r\n\r\n");
   ...
   memcpy(&sSendInfo.au8CdmaIndex[0], &au8Nbr[0], 11);
...
/*****************************************************************************
 *函数名称: CmdFlashLedDisable
 *功能描述: 关闭LED闪烁功能

 *参数:
 *返回:

发送： 0xAA + 0x55 + 0x18 + 0xCC + 0x33
*****************************************************************************/
void CmdFlashLedDisable(void)
{
    FlashLedDisable();
}
...
/*****************************************************************************
 *函数名称: CmdFlashLedEnable
 *功能描述: 打开LED闪烁功能

 *参数:
 *返回:

发送： 0xAA + 0x55 + 0x19 + 0xCC + 0x33
*****************************************************************************/
void CmdFlashLedEnable(void)
{
    FlashLedEnable();
}
...
void CmdHandlerInit(void)
{
  ...
  apCmdHandlerList[CMD_SET_SYS_INDEX    ]  = SetSystemType;             //设置系统编号
  apCmdHandlerList[CMD_GET_SYS_INDEX    ]  = GetSystemType;             //获取系统编号
  ...
  apCmdHandlerList[CMD_USE_AUTO_MODE    ]  = UseAutoMode;                //使用自动模式进行通讯
  ...
  apCmdHandlerList[CMD_FLASH_LED_ON     ]  = CmdFlashLedEnable;          //关闭
  apCmdHandlerList[CMD_FLASH_LED_OFF    ]  = CmdFlashLedDisable;         //打开
