...
UINT16 Uart2GetRxData(UINT8 *pu8Buffer, UINT16 u16DataLen)
{
  ...
  //若帧头到帧尾长度大于所要的长度
  if(u16HeaderToEndOfBuf > u16DataLen)//没循环，取在同一行
  {
    memcpy(pu8Buffer, sUart2Cb.pu8RxBuf+sUart2Cb.u16RxHeader, u16DataLen);//从缓冲区读datalen长度的数据到pu8buffer
    UART2_DISABLE_INTERRUPT();
    sUart2Cb.u16RxDataLen -= u16DataLen;//将数据长度减少datalen长度
    sUart2Cb.u16RxHeader += u16DataLen;//重新设置帧头
    UART2_ENABLE_INTERRUPT();
  }
  else//循环了，所取数据不在同一行
  {
    memcpy(pu8Buffer, sUart2Cb.pu8RxBuf+sUart2Cb.u16RxHeader, u16HeaderToEndOfBuf);//复制全部剩余的数据
    memcpy(pu8Buffer+u16HeaderToEndOfBuf, sUart2Cb.pu8RxBuf, u16DataLen - u16HeaderToEndOfBuf);//复制从数据帧开头的剩余数据
    //猜测缓冲区是个循环队列形式的
    UART2_DISABLE_INTERRUPT();
    sUart2Cb.u16RxDataLen -= u16DataLen;//从缓冲区取走数据后，减去相应的数据长度
    sUart2Cb.u16RxHeader =   u16DataLen - u16HeaderToEndOfBuf;//重新设置数据帧头，设置为之前的位置
    UART2_ENABLE_INTERRUPT();
   }
...
/****************************************************************************
  ...
  返回：无
****************************************************************************/
#pragma vector = USCI_A2_VECTOR //果然是循环队列的缓冲区
__interrupt void UART2_RXISR(void)
{
  UINT8 u8Temp;

  u8Temp = UCA2RXBUF;//读寄存器
//  UCA1TXBUF = u8Temp;
